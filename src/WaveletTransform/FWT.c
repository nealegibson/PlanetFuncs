...
********************************************************************/
int DoFWT(double *data, size_t size, int dorder)
{
	...
	//assign wavelet type
	if(dorder==2){
		wavelet = gsl_wavelet_alloc(gsl_wavelet_haar, dorder);}
	else if (dorder>=0){
		wavelet = gsl_wavelet_alloc(gsl_wavelet_daubechies, dorder);}
	else{ //use bspline with negative nos
		wavelet = gsl_wavelet_alloc(gsl_wavelet_bspline, -dorder);}
...
Data is replaced by its transform.
********************************************************************/
int DoIFWT(double *data, size_t size, int dorder)
{
	//check if size is a power of 2
	if(!IsPow2(size)){
